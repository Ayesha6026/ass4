...
class graph {
  ...
  public:
    ...
    vertex_descriptor insert_vertex(const VertexProperty& vp){ 
      ...
      vertex* new_vert = new vertex(++m_max_vd, vp);
      m_vertices.insert(new_vert);
	    return m_max_vd;
	}
    ...
    edge_descriptor insert_edge(vertex_descriptor sd, vertex_descriptor td,
        ...
        edge* new_edg = new edge(sd,td,ep);
        m_edges.insert(new_edg);
    ...
    void insert_edge_undirected(vertex_descriptor sd, vertex_descriptor td,
        const EdgeProperty& ep){
          ...
          insert_edge(td,sd,ep);//use current insert and make edges going both directions
          
			
	}
    void erase_vertex(vertex_descriptor vd){
      vertex del_vert(vd,VertexProperty());
      m_vertices.erase(&del_vert);
      
      
		
	}
    void erase_edge(edge_descriptor ed){
      //find edge based on edge descriptor then delete
      edge* del_edg = new edge(ed.first, ed.second, EdgeProperty());
      
      m_edges.erase(del_edg);
