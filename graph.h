...
public:
  ...
  vertex_descriptor insert_vertex(const VertexProperty &vp)
  {
    ...
    // Step 2: Add it to the m_vertices set
    m_vertices.insert(v);

    // Step 3: Update the maximum vertex descriptor and return it
    m_max_vd++;

    // Step 4: Return the m_max_vd
    return m_max_vd;
  }
  ...
  edge_descriptor insert_edge(vertex_descriptor sd, vertex_descriptor td, const EdgeProperty &ep)
  {
    ...
    // Step 1: Check if the source (sv) and target (tv) vertices exist
    auto sv_iter = find_vertex(sd);
    auto tv_iter = find_vertex(td);

    // Step 2: Display error message if either vertex does not exist
    if (sv_iter == m_vertices.end() || tv_iter == m_vertices.end())
    {
      throw std::invalid_argument("ERROR! : One of the vertices does not exist.");
    }

    // Step 3: Create a new edge
    auto e = new edge(sd, td, ep);

    // Step 4: Add the edge to the unordered sets of edges
    m_edges.insert(e);

    // Step 5: Add the edge to the adjacency lists of the source (sv) and target (tv) vertices
    (*sv_iter)->m_out_edges.insert(e);
    (*tv_iter)->m_out_edges.insert(e);

    // Return the edge pair
  ...
  void insert_edge_undirected(vertex_descriptor sd, vertex_descriptor td, const EdgeProperty &ep)
  {
    ...
    // Step 1: Create a new edge connecting the source (sv) and target (tv) vertices
    auto e = new edge(sd, td, ep);

    // Step 2: Add the new edge to the unordered set of edges
    m_edges.insert(e);

    // Step 3: Find and add the new edge to the adjacency lists of both vertices (sv and tv)
    auto s_v = find_vertex(sd);
    auto t_v = find_vertex(td);

    // Step 4: Add the edge to the adjacency lists of the source and target vertices
    (*s_v)->m_out_edges.insert(e);
    (*t_v)->m_out_edges.insert(e);
  }

  void erase_vertex(vertex_descriptor vd)
  {
    // Step 1: Find the vertex with vertex_descriptor vd
    auto v_iter = find_vertex(vd);

    // Step 2: Check if the vertex exists
    if (v_iter == m_vertices.end())
    {
      throw std::invalid_argument("ERROR! : Vertex does not exist.");
    }

    // Step 3: Remove all the edges connected to the vertex
    MyAdjEdgeContainer adj_edges;
    for (auto e_iter = m_edges.begin(); e_iter != m_edges.end(); ++e_iter)
    {
      if ((*e_iter)->source() == vd || (*e_iter)->target() == vd)
      {
        adj_edges.insert(*e_iter);
      }
    }
    for (auto e_iter = adj_edges.begin(); e_iter != adj_edges.end(); ++e_iter)
    {
      m_edges.erase(*e_iter);
    }

    // Step 4: Remove the vertex from the vertices list
    m_vertices.erase(v_iter);
  }

  void erase_edge(edge_descriptor ed)
  {
    // Step 1: Check if the edge exists
    auto e_iter = find_edge(ed);
    if (e_iter == m_edges.end())
    {
      return;
    }

    // Step 2: Check if the source and target vertices exist
    auto sv_iter = find_vertex(ed.first);
    auto tv_iter = find_vertex(ed.second);
    if (sv_iter == m_vertices.end() || tv_iter == m_vertices.end())
    {
      return;
    }

    // Step 3: Remove the edge from the adjacency lists of the source (sv) and target (tv) vertices
    (*sv_iter)->m_out_edges.erase(*e_iter);
    (*tv_iter)->m_out_edges.erase(*e_iter);

    // Step 4: Delete the edge from the list of edges
    m_edges.erase(e_iter);
  }
...
private:
  ...
  public:
    /// required constructors/destructors
    edge(vertex_descriptor s, vertex_descriptor t, const EdgeProperty &w) : m_source(s), m_target(t), m_property(w) {}
