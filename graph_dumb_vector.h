...
public:
  ...
  {
    // Step 1: Add the new vertex to the m_vertices storage
    m_vertices.push_back(new vertex(m_max_vd, vp));

    // Step 2: Increment m_max_vd
    m_max_vd++;

    // Step 3: Return the maximum vertex descriptor
    return m_max_vd;
  }
  ...
  edge_descriptor insert_edge(vertex_descriptor sd, vertex_descriptor td, const EdgeProperty &ep)
  {
    ...
    // Step 2: Add the edge to the m_edges storage
    m_edges.push_back(e);

    // Step 3: Add the edge to the out edges of the source vertex
    auto it = find_vertex(sd);
    (*it)->m_out_edges.push_back(e);

    // Step 4: Return the edge descriptor
    return {sd, td};
  }

  void insert_edge_undirected(vertex_descriptor sd, vertex_descriptor td, const EdgeProperty &ep)
  {
    // Step 1: Create two directed edges
    edge *e1 = new edge(sd, td, ep);
    edge *e2 = new edge(td, sd, ep);

    // Step 2: Add the edges to the edge storage
    m_edges.push_back(e1);
    m_edges.push_back(e2);

    // Step 3: Find the source and target vertices
    auto s_it = find_vertex(sd);
    auto t_it = find_vertex(td);

    // Step 4: Add the first edge to the out-edges of the source vertex
    (*s_it)->m_out_edges.push_back(e1);

    // Step 5: Add the second edge to the out-edges of the target vertex
    (*t_it)->m_out_edges.push_back(e2);
  }

  void erase_vertex(vertex_descriptor vd)
  {
    // Step 1: Find the vertex and return if it is not found
    auto vertex_iter = find_vertex(vd);
    if (vertex_iter == vertices_end())
    {
      // The vertex is not in the graph
      return;
    }
    // Step 2: Delete all edges associated with the vertex
    for (auto edge_iter = (*vertex_iter)->m_out_edges.begin();
         edge_iter != (*vertex_iter)->m_out_edges.end();)
    {
      erase_edge((*edge_iter)->descriptor());
    }
    // Step 3: Delete the vertex from the vertex storage
    m_vertices.erase(vertex_iter);
  }

  void erase_edge(edge_descriptor ed)
  {
    // Step 1: Find the edge
    auto e_it = find_edge(ed);

    // Step 2: If the edge is found
    if (e_it != m_edges.end())
    {
      // find the source vertex
      auto s_vd = ed.first;
      auto s_it = find_vertex(s_vd);

      if (s_it != m_vertices.end())
      {
        // Remove the edge from the adjacency list
        auto &s_adj_edges = (*s_it)->m_out_edges;
        s_adj_edges.erase(std::remove(s_adj_edges.begin(), s_adj_edges.end(), *e_it), s_adj_edges.end());
      }
      // Step 3: Delete the edge from the m_edges storage
      m_edges.erase(e_it);
    }
  }
