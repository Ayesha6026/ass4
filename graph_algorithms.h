...
// Comment this if you haven't boost installed
...
void depth_first_search(const Graph &g, ParentMap &p)
{
  ...
  for (vertex_iterator vi = g.vertices_cbegin(); vi != g.vertices_cend(); ++vi)
  {
    ...
    if (vertices_unexplored.count(vd))
    {
      dfs_visit(vd, g, p, edges_unexplored, vertices_unexplored);
    }
  }
}

template <typename Graph, typename ParentMap>
void dfs_visit(const typename Graph::vertex_descriptor &vd, const Graph &g, ParentMap &p,
               std::unordered_set<typename Graph::edge_descriptor, std::hash<typename Graph::edge_descriptor>> &edges_unexplored,
  ...
  for (adj_edge_iterator aei = v->begin(); aei != v->end(); ++aei)
  {
    ...
    if (el != edges_unexplored.end())
    {
      ...
      if (vertices_unexplored.count(t))
      {
        // discovery edge
        p[t] = v->descriptor();
        dfs_visit(t, g, p, edges_unexplored, vertices_unexplored);
      }
    }
  }
